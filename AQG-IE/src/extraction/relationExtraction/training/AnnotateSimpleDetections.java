...
public class AnnotateSimpleDetections {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < relations.length; i++) {
			...
			String rpref = prefix + relations[i];
			
			File f = new File(pW.getSelectedUsefulDocumentsForCollection(collection,relations[i], selectednumber));
			...
			for (int j = 0; j < lines.size(); j++) {
				...
				List<Pair<Tuple, String[]>> tuples = RDFPESExtractor.extract(model, relations[i]);
				...
				for (int k = 0; k < tuples.size(); k++) {
					...
					try {
						...
						doc.createContentTag(offset, offset+length, "DETECTION", true);
