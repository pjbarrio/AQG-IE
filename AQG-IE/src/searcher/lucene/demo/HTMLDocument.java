...
import searcher.lucene.indexer.LuceneIndexer;
...
public class HTMLDocument {
  ...
  public static Document Document(File f, String newName)
...
//     doc.add(new Field("contents", parser.getReader()));
    ...
    LynxCommandLineGenerator lclg = new LynxCommandLineGenerator();
    
    CommandLineExecutor cle = new CommandLineExecutor();
    
    doc.add(new Field("contents", new JavaSegmenterAnalyzer(Version.LUCENE_CURRENT,new File("stopWords.txt"),LuceneIndexer.we,LuceneIndexer.unique,LuceneIndexer.lowercase,LuceneIndexer.stemmed).tokenStream("contents", new StringReader(cle.getOutput(lclg.getLynxCommandLine(f,newName))))));
