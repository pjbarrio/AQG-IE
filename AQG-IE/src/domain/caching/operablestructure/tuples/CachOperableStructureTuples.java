...
import utils.counter.command.impl.PersistOperableStructureAndTupleCommand;
...
public class CachOperableStructureTuples {
	...
	public static void main(String[] args) throws InvalidFormatException, IOException, ClassNotFoundException, InterruptedException {
		...
		int firstRes = Integer.valueOf(args[9]);
		
		int lastRes = Integer.valueOf(args[10]);
		
		persistentWriter pW = PersistenceImplementation.getWriter();
		
		Counter counter = new Counter(storeAfter,new PersistOperableStructureAndTupleCommand(pW));
		
		int informationExtractionId = RelationConfiguration.getInformationExtractionSystemId(infEsys);
		
		int tuplesExtractionSystemId = RelationConfiguration.getCachedInformationExtractionSystemId(infEsys);
		...
		int experimentTuple = experiment + 45;
		...
		for (int i = 0; i < databases.size() && numberOfDatabases > 0; i++) {
			...
			if (fromPool){
				...
				pW.insertExperimentStatus(experimentTuple, databases.get(i).getId(), pW.getComputerName(), ExperimentStatusEnum.FINISHED);
			}
//			pW.reportExperimentStatus(experiment,databases.get(i).getId(),pW.getComputerName(),ExperimentStatusEnum.RUNNING);
			
			Map<Long,Set<CandidateSentence>> candidateSentences = loadCandidateSentences(pW,databases.get(i), relationConf,ce,informationExtractionId,experiment,fromPool,numberOfDocs, where, firstRes, lastRes);
			...
			for (int j = 0; j < runningInstances; j++) {
				...
				ts.add(new CachOperableStructureTuples().generateOperableStructures(relationExtractionSystem.get(j),structureConfigurations.get(j),databases.get(i),splits.remove(j),relationConf,ce, informationExtractionId, pW,counter, relName, tuplesExtractionSystemId));
			...
			if (thereWereMore && !fromPool){
				pW.makeExperimentAvailable(experiment, databases.get(i).getId());
				pW.makeExperimentAvailable(experimentTuple, databases.get(i).getId());
			}
				...
				pW.reportExperimentStatus(experimentTuple,databases.get(i).getId(),pW.getComputerName(),ExperimentStatusEnum.FINISHED);
			}
	...
	public static Map<Long,Set<CandidateSentence>> loadCandidateSentences(
			...
			ContentExtractor ce, int informationExtractionSystem, int idExperiment, boolean fromPool, int numDocs, String where, int firstRes, int lastRes) throws IOException, ClassNotFoundException {
		...
		if (fromPool){
			...
			candidateSentences = CreateOperableStructureSplit.loadNonProcessedDocsCandidateSentences(pW, database, relationConf, ce, informationExtractionSystem,cachedIeSys,where,true, firstRes, lastRes);
		}
		...
		for (Long idDoc : candidateSentences) {
			...
			if (val.first == 0){
				...
				Thread t = new Thread(new CachOperableStructureTuples().new CandidSentReadRunnable(idDoc,val.second,ret));
	...
	private Thread generateOperableStructures(Model model, StructureConfiguration structureConfiguration, Database database,
			Map<Long, Set<CandidateSentence>> candSents, int relationConf,
			ContentExtractor ce, int informationExtractionId, persistentWriter pW, Counter counter, String relation, int tuplesRelationExtraction) {
		
		Thread t = new Thread(new GenerateOperableStructuresRunnableTuples(pW,model,structureConfiguration,database,candSents,relationConf,ce,informationExtractionId,counter, relation, pW.getRelationExtractionSystemId(relationConf,tuplesRelationExtraction)));
